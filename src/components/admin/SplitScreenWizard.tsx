...
export const SplitScreenWizard: React.FC<SplitScreenWizardProps> = ({
  ...
  // Iframe & Extension Sync State
  const [iframeKey, setIframeKey] = useState<number>(0);
  const [isIframeLoading, setIsIframeLoading] = useState<boolean>(true);
  const [externalTabStatus, setExternalTabStatus] = useState<string>('Listening for External Tab Stream...');
  const [isConnectedToExternalTab, setIsConnectedToExternalTab] = useState<boolean>(false);
  const [recordedJourneyLogs, setRecordedJourneyLogs] = useState<Array<{ id: string; text: string; time: string }>>([]);
  ...
  // BroadcastChannel for syncing with external tab (Approach C Companion)
  useEffect(() => {
    const channel = new BroadcastChannel('lynqly_live_stream_channel');

    channel.onmessage = (e) => {
      if (e.data) {
        if (e.data.type === 'LYNQLY_TAB_HEARTBEAT') {
          setIsConnectedToExternalTab(true);
          setExternalTabStatus(`🟢 Connected to External Tab: ${e.data.title || e.data.url}`);
        } else if (e.data.type === 'LYNQLY_ELEMENT_RECORDED') {
          const text = e.data.text?.trim() || 'Selected Element';
          const time = new Date().toLocaleTimeString();

          setRecordedJourneyLogs((prev) => [
            { id: Date.now().toString(), text: `Recorded click on "${text}" (${e.data.tagName})`, time },
            ...prev
          ]);

          // Automatically record into Step 3 field list
          setCapturedFields((prev) => [
            ...prev,
            { name: text.substring(0, 30) || 'Captured Field', type: 'TEXT', sample: text.substring(0, 50) }
          ]);
        }
      }
    };
    ...
    <div className="fixed inset-0 z-50 flex flex-col font-sans bg-slate-100 text-slate-900">
      ...
      <header className="px-6 py-3 bg-white border-b border-slate-200 flex items-center justify-between shrink-0 shadow-sm relative z-10">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-4 py-2 bg-gradient-to-r from-indigo-600 to-cyan-600 hover:opacity-95 text-white font-black text-xs rounded-xl shadow flex items-center gap-2 transition-all"
          >
            <ExternalLink className="w-3.5 h-3.5" /> ↗ Open Target Site in Monitored Separate Tab
          </button>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-full lg:w-[60%] flex flex-col justify-between p-4 space-y-3 bg-slate-100">
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              {targetUrl && (
                <button
                  ...
                  className="p-1.5 bg-indigo-50 border border-indigo-200 text-indigo-700 hover:bg-indigo-100 rounded-lg transition-all flex items-center gap-1 text-[11px] font-extrabold"
                >
          ...
          {/* EXTERNAL TAB MONITORED STREAM BADGE */}
          <div className="px-4 py-2 bg-white border border-indigo-200 rounded-xl flex items-center justify-between text-xs font-bold shadow-sm">
            <span className="flex items-center gap-2 text-indigo-900">
              <Radio className={`w-4 h-4 ${isConnectedToExternalTab ? 'text-emerald-600 animate-pulse' : 'text-amber-500'}`} />
              {externalTabStatus}
            </span>
            <a
              href="/extension/manifest.json"
              download
              className="text-[11px] text-indigo-600 hover:underline font-extrabold flex items-center gap-1"
            >
              <Download className="w-3 h-3" /> Extension Companion Ready
            </a>
          </div>
          ...
          <div className="flex-1 rounded-2xl border border-slate-200 bg-white p-2 overflow-hidden relative shadow-md flex flex-col">
            ...
            <div className="px-3 py-2 border-b border-slate-100 flex items-center justify-between text-xs shrink-0">
              ...
              <span className="px-2.5 py-0.5 bg-emerald-50 border border-emerald-300 text-emerald-700 text-[10px] font-black rounded-full flex items-center gap-1">
                <span className="w-2 h-2 rounded-full bg-emerald-500 animate-ping" /> SYNC ACTIVE
              </span>
            </div>
                ...
                <>
                  {isIframeLoading && (
                    <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-white/90 backdrop-blur-sm space-y-3">
                      ...
                      <p className="text-xs font-black text-slate-700">Connecting Proxy Stream to {targetUrl}...</p>
                    </div>
                  )}
                  <iframe
                    key={iframeKey}
          ...
          {/* REAL-TIME JOURNEY LOG FEED */}
          <div className="p-3 bg-white border border-slate-200 rounded-2xl space-y-2 text-xs shadow-sm max-h-28 overflow-y-auto font-mono">
            <div className="flex items-center justify-between text-[11px] font-black text-slate-700 border-b border-slate-100 pb-1">
              <span>LIVE RECORDED JOURNEY ACTIVITIES ({recordedJourneyLogs.length})</span>
              <span className="text-emerald-600">REALTIME SYNC</span>
            </div>
            {recordedJourneyLogs.length === 0 ? (
              <p className="text-[11px] text-slate-400 font-sans italic">No clicks recorded yet. Click any element inside the left stream or in your separate target tab.</p>
            ) : (
              recordedJourneyLogs.map((log) => (
                <div key={log.id} className="flex items-center justify-between text-[11px] text-indigo-900 font-semibold">
                  <span>⚡ {log.text}</span>
            ...
            {step === 2 && (
              <div className="space-y-6">
                <div className="p-5 bg-white border border-indigo-200/90 rounded-3xl space-y-1.5 shadow-sm">
                  ...
                  <p className="text-xs text-slate-600 leading-relaxed font-semibold">
                    Click on any item card on the left preview screen or in your open target tab to lock recurring item boundaries.
                  </p>
                </div>
            ...
            {step === 3 && (
              <div className="space-y-6">
                <div className="p-5 bg-white border border-indigo-200/90 rounded-3xl space-y-1.5 shadow-sm">
                  ...
                  <p className="text-xs text-slate-600 leading-relaxed font-semibold">
                    Click any element inside the stream or in your separate target tab to capture fields automatically.
                  </p>
                </div>
